...
export const MostPopularCategory = () => {
  ...
  const renderContent = (
    names?.map(name => (
      <Link className="text-center bg-slate-600 px-4 text-slate-200 hover:text-slate-400 xxs:w-fit sm:w-48" title={t(`${name}`)} onClick={() => handleClick(name)} key={name} to={`/categories/${name || "Beef"}`}>{t(`${name}`) || "Beef"}</Link>
    ))
  )
